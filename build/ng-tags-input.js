 ...
 * License: MIT
 *
 * Generated at 2019-07-24 10:38:54 +0300
 */
(function (angular$1) {
'use strict';
  ...
  return {
    ...
    link: function link(scope, element, attrs, ngModelCtrl) {
      ...
      events.on('tag-added', scope.onTagAdded).on('invalid-tag', scope.onInvalidTag).on('tag-removed', scope.onTagRemoved).on('tag-clicked', scope.onTagClicked).on('tag-added', function () {
        ...
        var shouldRemove = (key === tiConstants.KEYS.backspace || key === tiConstants.KEYS.delete) && scope.newTag.text().length === 0 && tagList.items.length > 0;
        ...
        if ((options.addOnSpace && event.originalEvent.data === ' ' || options.addOnComma && event.originalEvent.data === ',') && scope.text && scope.text.endsWith(event.originalEvent.data)) {
...
function TemplateCacheRegister($templateCache) {
  ...
  $templateCache.put('ngTagsInput/tags-input.html', "<div class=\"host\" tabindex=\"-1\" ng-click=\"eventHandlers.host.click()\" ti-transclude-append><div class=\"tags\" ng-class=\"{focused: hasFocus}\"><ul class=\"tag-list\"><li class=\"tag-item\" ng-repeat=\"tag in tagList.items track by track(tag)\" ng-class=\"getTagClass(tag, $index)\" ng-click=\"eventHandlers.tag.click(tag)\"><ti-tag-item scope=\"templateScope\" data=\"::tag\"></ti-tag-item></li></ul><input class=\"input\" autocomplete=\"off\" ng-model=\"newTag.text\" ng-model-options=\"{getterSetter: true}\" ng-keydown=\"eventHandlers.input.keydown($event)\" ng-on-input=\"eventHandlers.input.input($event)\" ng-focus=\"eventHandlers.input.focus($event)\" ng-blur=\"eventHandlers.input.blur($event)\" ng-paste=\"eventHandlers.input.paste($event)\" ng-trim=\"false\" ng-class=\"{'invalid-tag': newTag.invalid}\" ng-disabled=\"disabled\" ti-bind-attrs=\"{type: options.type, placeholder: options.placeholder, tabindex: options.tabindex, spellcheck: options.spellcheck}\" ti-autosize></div></div>");
}
